...
PREFIX       = 'NH3NMBR'
MODEL        = 'XGBoost'
TARGET_LABEL = r'$\mathrm{NH_3}$-$\mathrm{N_{MBR}}$'
...
# -----------------------------------------------------------------------
pair_files = sorted(Path("./data/8").glob(f"{PREFIX}_{MODEL}_*_x_*.csv"))
...
plt.tight_layout()
plt.savefig(f"figs/8_{PREFIX}_interaction_beeswarm.png", dpi=300, bbox_inches='tight')
plt.close(fig)
...
plt.savefig(f"figs/8_{PREFIX}_interaction_pairs.png", dpi=300, bbox_inches='tight')
plt.close(fig)
