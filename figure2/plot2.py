...
for it, method in enumerate(methods):
    ...
    # Load CSV
    df = pd.read_csv(f"./data/2/{method}.csv")
    ...
    ax1.set_xticklabels([])
    ax2.set_xticklabels([])
    ax1.set_ylabel(df.columns[1] if df.columns[1] != 'CODMBR' else r'$\mathrm{COD_{MBR}}$')
    ax2.set_ylabel(df.columns[2] if df.columns[2] != 'CODMBR' else r'$\mathrm{COD_{MBR}}$')
    ax3.set_ylabel(df.columns[3] if df.columns[3] != 'CODMBR' else r'$\mathrm{COD_{MBR}}$')
    ...
    # plt.show()
    plt.savefig(f"figs/2_{method}.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
