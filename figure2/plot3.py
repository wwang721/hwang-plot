...
for it, method in enumerate(methods):
    ...
    # Load CSV
    df = pd.read_csv(f"./data/3/{method}.csv")

    time = pd.to_datetime(df.iloc[:, 0])
    value_names = ['Date', r'$\mathrm{NH_3}$-$\mathrm{N_{MBR}}$', method, 'Upper threshold', 'Lower threshold', f'{method} flag']
    ...
    for idx in range(4):
        ...
        if idx == 0:
            ax1.plot(time, value, clip_on=False, color='C0', label=r'Raw $\mathrm{NH_3}$-$\mathrm{N_{MBR}}$', alpha=alpha, lw=lw)
        else:
    ...
    # plt.show()
    plt.savefig(f"figs/3_{method}.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
