...
plots = [
    ('subtrain_raw_target_acf_plot.csv',   'acf',  'Autocorrelation',         r'ACF of Subtrain Raw Target Series ($\mathrm{NH_3}$-$\mathrm{N_{MBR}}$)',         '(a)'),
    ('subtrain_raw_target_pacf_plot.csv',  'pacf', 'Partial Autocorrelation', r'PACF of Subtrain Raw Target Series ($\mathrm{NH_3}$-$\mathrm{N_{MBR}}$)',        '(b)'),
    ('subtrain_target_diff1_acf_plot.csv', 'acf',  'Autocorrelation',         r'ACF of Subtrain Diff-1 Target Series ($\mathrm{NH_3}$-$\mathrm{N_{MBR}}$)',  '(c)'),
    ('subtrain_target_diff1_pacf_plot.csv','pacf', 'Partial Autocorrelation', r'PACF of Subtrain Diff-1 Target Series ($\mathrm{NH_3}$-$\mathrm{N_{MBR}}$)', '(d)'),
]

fig, axes = plt.subplots(2, 2, figsize=(14, 7))

for ax, (fname, col, ylabel, title, panel_label) in zip(axes.flatten(), plots):
    df = pd.read_csv(f"./data/4/{fname}")
    lags   = df['lag'].values
...
plt.savefig("figs/4_NH3N_acf_pacf.png", dpi=300, bbox_inches='tight')
plt.close(fig)
