...
models = [
    ('NH3N-naive',          'Naive Baseline',   False),
    ('NH3N-ridge',          'Ridge Regression', False),
    ('NH3N-weighted',       'Weighted Average', False),
    ('NH3N-lstm',           'LSTM',             False),
    ('NH3N-TCAN-ATTENTION', 'TCAN-Attention',   True),
]
...
for it, (fname, label, is_tcan) in enumerate(models):
    df = pd.read_csv(f"./data/2/{fname}.csv")
    ...
    if is_tcan:
        ...
        time     = pd.to_datetime(df['target_time'])
        y_true   = df['y_true']
        y_pred   = df['y_pred']
        residual = df['residual']
        split    = df['dataset']
    ...
    ax1.set_xticklabels([])
    ax1.set_ylabel(r'$\mathrm{NH_3}$-$\mathrm{N_{MBR}}$ (mg/L)', labelpad=12)
    ...
    plt.savefig(f"figs/2_{fname}.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
